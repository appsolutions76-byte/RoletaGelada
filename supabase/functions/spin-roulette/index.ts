...
serve(async (req) => {
  ...
  try {
    ...
    if (action === 'stop') {
      ...
      if (currentBalance >= targetBalance) {
        // O Cofre atingiu a meta (Markup Multiplicador configurado no Master). 
        // Libera o prêmio IMEDIATAMENTE (100% de chance) no exato momento que o usuário clicar em "Parar".
        won = true;
      }
